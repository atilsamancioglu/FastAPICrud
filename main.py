...
async def read_book(course_title: str):
    for course in courses_db:
        if course.get('title').casefold() == course_title.casefold():
            return course

@app.get("/courses/{course_id}")
async def read_book(course_id: str):
    for course in courses_db:
...
async def get_courses_by_author_path(instructor: str):
    courses_to_return = []
    for course in courses_db:
...
async def read_author_category_by_query(course_instructor: str, category: str):
    courses_to_return = []
    for course in courses_db:
...
async def create_book(new_course=Body()):
    courses_db.append(new_course)


@app.put("/courses/update_course")
async def update_book(updated_course=Body()):
...
async def delete_book(course_id: str):
    for i in range(len(courses_db)):
